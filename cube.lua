...
return function(mvp, pData, x, y, z)
	...
	--glasses.drawRightboundString(5, 5, (textutils.serialize(pData.lookingAt):gsub("%s+", " ")))
	--glasses.addText(5, 15, (textutils.serialize(pData.position):gsub("%s+", " ")))
	glasses.drawRightboundString(5, 25, (textutils.serialize({yaw = pData.yaw, pitch = pData.pitch}):gsub("%s+", " ")))
end